...
function createGame(roomCode, hostId, hostName) {
  return {
    id: uuidv4(),
    roomCode,
    phase: "waiting",
    players: [{
      ...
      color: PLAYER_COLORS[0], avatar: PLAYER_AVATARS[0],
...
function advanceTurn(state) {
  ...
  if (newRound > state.maxRounds) {
    ...
    return { ...state, phase: "finished", winner: winner?.id, round: newRound,
...
function processLanding(state, playerId, position) {
  ...
  switch (tile.type) {
    ...
    case "treasure": {
      ...
      return { ...state, phase: "card", currentCard: { type: "treasure", card },
    ...
    case "surprise": {
      ...
      return { ...state, phase: "card", currentCard: { type: "surprise", card },
    ...
    case "go-to-prison": {
      ...
      return { ...state, players, phase: "rolling",
...
function handleBankruptcy(state, bankruptId, creditorId) {
  ...
  return { ...state, players, properties, phase: winner ? "finished" : "rolling", winner,
    log: [...state.log,
...
io.on("connection", (socket) => {
  ...
  socket.on("create-room", ({ playerName }) => {
    try {
      ...
      const state = createGame(roomCode, socket.id, playerName);
  ...
  socket.on("start-game", () => {
    try {
      ...
      state = { ...state, phase: "rolling", round: 1,
  ...
  socket.on("roll-dice", () => {
    try {
      ...
      let player = { ...cur, lastDice: [d1, d2],
        consecutiveDoubles: isDouble ? cur.consecutiveDoubles + 1 : 0 };

      // 3 doubles = jail
      if (player.consecutiveDoubles >= 3) {
        player = sendToJail(player);
        state = { ...state, players: state.players.map(p => p.id === socket.id ? player : p),
  ...
  socket.on("decline-purchase", () => {
    try {
      ...
      state = { ...state, phase: "auction",
        currentAuction: { tileId: cur.position, currentBid: 0, currentBidderId: null, bids: [],
  ...
  socket.on("auction-bid", ({ amount }) => {
    try {
      ...
      state = { ...state, currentAuction: { ...state.currentAuction,
  ...
  function finishAuction(roomCode) {
    ...
    if (!auction.currentBidderId) {
      ...
      state = advanceTurn({ ...state, players,
  ...
  socket.on("build-house", ({ tileId }) => {
    try {
      ...
      state = { ...state, players, properties,
  ...
  socket.on("respond-trade", ({ accept }) => {
    try {
      ...
      if (accept) {
        const players = state.players.map(p => {
          if (p.id === trade.fromPlayerId) return { ...p,
          ...
          if (p.id === trade.toPlayerId) return { ...p,
        ...
        state = { ...state, players, properties, pendingTrade: null, phase: "rolling",
  ...
  socket.on("send-chat", ({ message }) => {
    try {
      ...
      const msg = { id: uuidv4(), playerId: socket.id, playerName: player.name,
